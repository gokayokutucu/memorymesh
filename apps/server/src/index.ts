...
function createServer(): McpServer {
  ...
  const registrationPlan = getMemoryToolRegistrationPlan();

  server.tool(
    "get_runtime_health",
    "Return runtime health diagnostics for persistence backends and MCP tool visibility. Use this for diagnosing mode/tool registration, transient store degradation, and circuit-breaker state.",
    {},
    async () => {
      const runtimeHealth = getRuntimeHealth();
      const payload = {
        mode: registrationPlan.mode,
        read_tools_enabled: registrationPlan.readToolsEnabled,
        write_tools_enabled: registrationPlan.writeToolsEnabled,
        registered_tools: getRegisteredToolNames(registrationPlan),
        stores: runtimeHealth,
      };
      return {
        content: [{ type: "text", text: formatRuntimeHealthToolText(payload) }],
      };
    }
  );

  if (registrationPlan.writeToolsEnabled) {
    server.tool(
      "save_memory",
      ...
      async ({
        ...
        return {
          content: [{ type: "text", text: formatSaveMemoryToolText(result) }],
        };
      }
    );
  }
  ...
  if (registrationPlan.readToolsEnabled) {
    server.tool(
      "search_memory",
      ...
      async ({ query, project, limit, tags, ref_id, title, source_type, sort_by, before, after }) => {
        ...
        return {
          content: [{ type: "text", text: formatSearchMemoryToolText(results) }],
        };
      }
    );
  }
...
async function main() {
  const registrationPlan = getMemoryToolRegistrationPlan();
  for (const line of formatMemoryMcpModeSummary(registrationPlan)) {
    console.error(line);
  }
